...
import { Contract, ethers, providers, Transaction, Wallet } from "ethers";
...
export class ItsRelayerService {
    ...
    private readonly relayerWhitelistedChains: string[];
    ...
    constructor(@Inject(ConfigService) private readonly configService: ConfigService) {
        ...
        this.relayerWhitelistedChains = this.configService.get<string[]>("axelar.relayerWhitelistedChains")!;
    }
     ...
     */
    async verifyMessages(relayerRequest: RelayerEvmRequest): Promise<any> {
    ...
    async executeItsHub(relayerRequest: RelayerEvmRequest): Promise<RelayerEvmRequest> {
        ...
        const contractCallLog = jsonResponse.logs[0].events.find((log) => log.type === "wasm-contract_called");
        return {
            destinationAddress: contractCallLog.attributes.find((attr) => attr.key === "destination_address").value,
            destinationChain: contractCallLog.attributes.find((attr) => attr.key === "destination_chain").value,
            messageId: contractCallLog.attributes.find((attr) => attr.key === "message_id").value,
            payload: contractCallLog.attributes.find((attr) => attr.key === "payload").value,
            payloadHash: contractCallLog.attributes.find((attr) => attr.key === "payload_hash").value,
            sourceAddress: contractCallLog.attributes.find((attr) => attr.key === "source_address").value,
            sourceChain: contractCallLog.attributes.find((attr) => attr.key === "source_chain").value,
        };
    }
     ...
     */
    async prepareRelayTransferTransaction(relayerRequest: RelayerEvmRequest, multisigSessionId: string): Promise<Partial<Transaction>> {
        const destinationChainMultisigProver = axelarChains.axelar.contracts.MultisigProver[relayerRequest.destinationChain].address;
     ...
     */
    async signAndSubmitTransaction(chain: string, transaction: Partial<Transaction>): Promise<void> {
    ...
    async resolveEvmTransaction(
        ...
        if (this.relayerWhitelistedChains.indexOf(relayerRequest.sourceChain) === -1) {
            await this.verifyMessages(relayerRequest);
        }
        await sleep(this.verifyWaitTime);
        const itsRelayerRequest = await this.executeItsHub(relayerRequest);
        const multisigSessionId = await this.constructTransferProof(itsRelayerRequest);
        await sleep(this.proveWaitTime);
        return { relayerRequest: itsRelayerRequest, multisigSessionId };
    }
    ...
    async relayEvmToEvm(relayerRequest: RelayerEvmRequest): Promise<void> {
        const { relayerRequest, multisigSessionId } = await this.resolveEvmTransaction(relayerRequest);
        await this.relayTransactionToEvm(relayerRequest, multisigSessionId);
    }
}
